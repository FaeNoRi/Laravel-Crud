...
<div class="card uper">
  ...
  <div class="card-body">
    ...
    @endif
      <form method="post" action="{{ route('fiches.update', $fichescases->id ) }}">
        <div class="form-group">
            @csrf
            @method('PATCH')
            <label for="nom">Nom :</label>
            <input type="text" class="form-control" name="nom" value="{{$fichescases->nom}}"/>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control" name="sortie" value="{{$fichescases->sortie}}"/>
        </div>
        <div class="form-group">
          <label for="synopsis">Synopsis :</label>
          <textarea rows="5" columns="5" class="form-control" name="synopsis"> {{$fichescases->synopsis}} </textarea>
        </div>
